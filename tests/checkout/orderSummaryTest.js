...
describe('test suite: renderOrderSummary', () => {
    ...
    // hook - code to be run applying to all tests, jasmine has 4 types of hooks: beforeEach() afterEach() beforeAll() afterAll()
    ...
    it('displays the cart', () => {
        ...
        expect(document.querySelector(`.js-product-name-${productId1}`).innerHTML
        ).toContain('Black and Gray Athletic Cotton Socks - 6 Pairs');

        expect(document.querySelector(`.js-product-price-${productId1}`).innerHTML
        ).toContain('$10.90');
    })
    it('removes a product', () => {
        document.querySelector(`.js-delete-link-${productId1}`).click();
        // check if length of cart [] has changed from 2 to 1
        expect(document.querySelectorAll('.js-cart-item-container').length
        ).toEqual(1);
        // check if product one has been deleted (will return null)
        expect(document.querySelector(`.js-cart-item-container-${productId1}`)
        ).toEqual(null);
        // check if product two still exists (is not null)
        expect(document.querySelector(`.js-cart-item-container-${productId2}`)
        ).not.toEqual(null);
        expect(cart.length).toEqual(1);
        expect(cart[0].productId).toEqual(productId2);
        expect(document.querySelector(`.js-product-name-${productId2}`).innerHTML
